...
describe('структурные ключевые слова внутри «сырых» выражений (полный аудит парсера)', () => {
  ...
  // 7-е место того же класса бага, найденное адверсарной перепроверкой уже
  // исправленных пунктов (не входило в первый проход аудита): выражение-колонка
  // в проекции табличной части (`Т.ТЧ.(…)`) читалось сырым срезом без стоп-слов,
  // и `ИЗ`/`ПОМЕСТИТЬ` внутри неё молча становились частью «произвольного»
  // выражения колонки вместо синтаксической ошибки.
  it('проекция табличной части Т.ТЧ.(…) проглатывает ИЗ (parseTabColumn)', () => {
    const text = 'ВЫБРАТЬ Т.Товары.(Номенклатура, ИЗ Прочее) КАК ТЧ ИЗ Документ.РеализацияТоваровУслуг КАК Т';
    expect(tryOpenBatch(text).ok).toBe(false);
  });
...
describe('глубина рекурсии вложенных подзапросов', () => {
  // Без ограничения глубокая вложенность роняла процесс по памяти (heap out of
  // memory, не перехватывается никаким try/catch) вместо обычной, перехватываемой
  // ошибки парсера — каждый уровень заново токенизирует и разбирает почти весь
  // остаток текста. Максимум по золотому корпусу — 3; лимит — 32.
  function nestedFromSource(depth: number): string {
    let text = 'ВЫБРАТЬ Т.Ссылка ИЗ Справочник.Валюты КАК Т';
    for (let i = 0; i < depth; i++) {
      text = `ВЫБРАТЬ Т.Ссылка ИЗ (${text}) КАК Т`;
    }
    return text;
  }

  it('вложенность в пределах лимита разбирается как обычно (ok:true)', () => {
    expect(tryOpenBatch(nestedFromSource(20)).ok).toBe(true);
  });

  it('вложенность далеко за пределами лимита — обычная, перехватываемая ошибка (ok:false), без падения процесса', () => {
    const result = tryOpenBatch(nestedFromSource(100));
