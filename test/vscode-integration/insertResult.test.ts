...
describe('Extension Host: insertResult — реальная замена BSL-литерала', () => {
  ...
  // Post-release audit P1 №4 (TextEditor.edit() возвращает false → раньше текст
  // молча терялся): само ветвление "какую причину clipboard-фолбека выбрать"
  // покрыто чистым юнит-тестом test/unit/clipboardFallback.test.ts — реальный
  // `TextEditor.edit()` не даёт детерминированно вызвать `false` в Extension Host
  // (это read-only свойство прокси-объекта, не переопределить; а версионная/
  // видимостная гонка, которую сам API документирует как причину `false`, — не
  // то, что можно надёжно спровоцировать в тесте).
});
