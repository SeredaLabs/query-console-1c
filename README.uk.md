...
| 🔍 **Підказки, автодоповнення та діагностика** | Підказки по полях/таблицях/віртуальних таблицях/параметрах, автодоповнення по `.`/`&` і попередження про розбір — прямо в тексті запиту, без конструктора |
...
## 🧪 Експериментальний Новий конструктор

Новий візуальний конструктор ще розробляється і типово вимкнений. Щоб його
тестувати, увімкніть `queryConsole.enableNewBuilderPreview` у Налаштуваннях VS
Code, а потім виконайте **1C: Новий конструктор (Preview)** у палітрі команд.
Він не замінює стабільний Classic Constructor.
...
| [Підказки, автодоповнення та діагностика](docs/uk/hover-and-completion.md) | [Відомі проблеми англійською](docs/development/known-issues.md) |
| [Усунення проблем](docs/uk/troubleshooting.md) | |
...
Проєкт розпочався у серпні 2026 року як форк
[AlekseyUAM/query_console_vscode](https://github.com/AlekseyUAM/query_console_vscode).
Відтоді рушій запитів, модель метаданих і перевірку переписали, а також
з'явився цілком новий семантичний шар — підказки, автодоповнення та
діагностика, — якого не було в оригіналі. Тепер це незалежний проєкт, який
підтримує SeredaLabs.
