...
- **Параметры некоторых виртуальных таблиц теряются при `parse -> generate`
  ...
  регистров расчёта/последовательностей вовсе — тот "золотой" (harvested-from-
  real-1C) пайплайн (`extract`→`harvest`→`accept:oracle`) требует настоящего
  запущенного 1С через MCP-оракул для эталонного текста, недоступного в этой
  сессии; выдумывать такой эталон нельзя (тот же принцип, что и у самого этого
  пункта). Вместо этого разовая проверка 2026-09-05 закреплена как переиспользуемый
  инструмент: `src/cli/scanUnsafeVirtualTables.ts` (`npm run corpus:scan-unsafe-vt`,
  `CONFIG_DIR=/path/to/real/cf`) прогоняет `extractQueryStrings`/`findUnsafeVirtualTables`
  по ЛЮБОЙ реальной выгрузке и сообщает о находках с ненулевым кодом выхода —
  не встроен в CI (нет доступа к реальной выгрузке там), но теперь воспроизводим
  на любой будущей конфигурации одной командой, а не разовым scratch-скриптом.
  Логика самого сканера (обход/агрегация) покрыта `test/unit/scanUnsafeVirtualTables.test.ts`
  на синтетических фикстурах — эти НЕ заменяют проверку на реальных данных.
  Владельцы: `src/core/query/sdblParser.ts`, `semanticValidator.ts`,
  `src/cli/scanUnsafeVirtualTables.ts`.
