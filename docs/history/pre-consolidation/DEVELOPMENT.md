...
Это производный cache: не коммитьте его и обновляйте после изменения XML.

Импорт (PR-02, `src/core/metadata/parser/parseConfiguration.ts`) собирает новую
генерацию в отдельном staging-каталоге и коммитит её одной операцией только после
успешной сборки — предыдущая генерация остаётся нетронутой при любом сбое.
Каталог, не распознанный как созданный этим расширением (нет маркера владения),
никогда не удаляется — новая генерация коммитится рядом, в `cf-managed`. Отдельные
XML с ошибкой пропускаются (видны в диагностике), не прерывая импорт целиком.
Остающиеся ограничения (устаревание cache без сверки с XML, symlink-каталоги вне
auto-discovery) -- см. [CORE_LIMITATIONS.md](CORE_LIMITATIONS.md) и
[KNOWN_ISSUES.md](KNOWN_ISSUES.md).

Измеренные величины сборки/загрузки метаданных, размера WebView payload и
parse/generate по корпусу -- [PERFORMANCE_BASELINE.md](PERFORMANCE_BASELINE.md).
...
[history/](../README.md) только как архив доказательств.
