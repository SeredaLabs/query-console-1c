...
- **Параметры некоторых виртуальных таблиц теряются при `parse -> generate`.**
  ...
  `[period, condition]` вслепую было бы её выдумыванием без основания. До
  исправления или до Apply-blocking для этого случая (см. PR-05 «Apply
  Safety» в ТЗ §54 P0.5) — не редактировать такие источники через конструктор
  при 3+ аргументах. Владельцы: `src/core/query/sdblParser.ts`.
