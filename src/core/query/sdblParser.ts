...
function parseVirtualParams(cur: Cursor, fullName: string, tableIndex: number): VirtualParams {
  ...
  // РегистрРасчета: раскладка подтверждена (Хрусталёва, «Язык запросов
  // "1С:Предприятия 8"», 2-е изд., с. 327-334 — не догадка, свежепроверено).
  if (kind === 'РегистрРасчета') {
    if (slice === 'ФактическийПериодДействия' || slice === 'ДанныеГрафика') {
      // [condition] — единственный параметр, арность 1.
      set('condition', arg(args, 0));
      if (args.slice(1).some(a => a !== '')) v.unsafeExtraArgs = true;
      return v;
    }
    if (slice.startsWith('База')) {
      // <ОсновнойРегистр>.База<БазовыйРегистр>(ИзмеренияОсновногоРегистра,
      // ИзмеренияБазовогоРегистра, Разрезы, Условие) — арность 4.
      set('mainDimensions', arg(args, 0));
      set('baseDimensions', arg(args, 1));
      set('sections', arg(args, 2));
      set('condition', arg(args, 3));
  ...
  // арностью 3+, чья раскладка НЕ подтверждена (`Последовательность.*.Границы`
  // — см. KNOWN_ISSUES.md; регистр расчета выше уже подтверждён и выделен
  // отдельно) непустой аргумент на позиции 2+ молча терялся бы при generate —
  // помечаем как unsafeExtraArgs, чтобы Apply мог заблокировать запись
  // (PR-05, ТЗ §54 P0.5).
