...
| 🔍 **Подсказки, автодополнение и диагностика** | Подсказки по полям/таблицам/виртуальным таблицам/параметрам, автодополнение по `.`/`&` и предупреждения о разборе — прямо в тексте запроса, без конструктора |
...
## 🧪 Экспериментальный Новый конструктор

Новый визуальный конструктор ещё находится в разработке и по умолчанию
отключён. Чтобы его тестировать, включите `queryConsole.enableNewBuilderPreview`
в Настройках VS Code, затем выполните **1C: Новый конструктор (Preview)** в
палитре команд. Он не заменяет стабильный Classic Constructor.
...
| [Подсказки, автодополнение и диагностика](docs/ru/hover-and-completion.md) | [Известные проблемы на английском](docs/development/known-issues.md) |
| [Устранение проблем](docs/ru/troubleshooting.md) | |
...
Проект начался в августе 2026 года как форк
[AlekseyUAM/query_console_vscode](https://github.com/AlekseyUAM/query_console_vscode).
С тех пор движок запросов, модель метаданных и проверка были переписаны, а
также появился совершенно новый семантический слой — подсказки,
автодополнение и диагностика, — которого не было в оригинале. Теперь это
независимый проект, который поддерживает SeredaLabs.
